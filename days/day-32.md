# Day 32: Multi-stage Builds

## Goal
Learn the core ideas for **multi-stage builds** and apply them to the course project.

## Topics
- Builder image
- Runtime image
- Reducing attack surface

## Hands-on Task
Refactor to a multi-stage build

## Deliverable
A smaller production image.

## Suggested Time
- Theory: 30–45 minutes
- Hands-on: 60–120 minutes
- Review: 15 minutes

## Git Commit Suggestion
```bash
git add .
git commit -m "day 32: multi-stage builds"
```

## Notes
Use this page to record:
- what you built
- what broke
- what you fixed
- screenshots
- links to commits and pull requests

## End-of-Day Reflection
1. What did I secure today?
2. What is still risky?
3. What should I automate next?
