# Day 12: Password Security

## Goal
Learn the core ideas for **password security** and apply them to the course project.

## Topics
- Hashing
- Salting
- BCrypt

## Hands-on Task
Store passwords with BCrypt

## Deliverable
A safe user registration flow.

## Suggested Time
- Theory: 30–45 minutes
- Hands-on: 60–120 minutes
- Review: 15 minutes

## Git Commit Suggestion
```bash
git add .
git commit -m "day 12: password security"
```

## Notes
Use this page to record:
- what you built
- what broke
- what you fixed
- screenshots
- links to commits and pull requests

## End-of-Day Reflection
1. What did I secure today?
2. What is still risky?
3. What should I automate next?
