# Day 28: Week 4 Mini Project

## Goal
Learn the core ideas for **week 4 mini project** and apply them to the course project.

## Topics
- Security posture
- Exceptions
- Documentation

## Hands-on Task
Produce a weekly security summary

## Deliverable
A markdown report with findings and fixes.

## Suggested Time
- Theory: 30–45 minutes
- Hands-on: 60–120 minutes
- Review: 15 minutes

## Git Commit Suggestion
```bash
git add .
git commit -m "day 28: week 4 mini project"
```

## Notes
Use this page to record:
- what you built
- what broke
- what you fixed
- screenshots
- links to commits and pull requests

## End-of-Day Reflection
1. What did I secure today?
2. What is still risky?
3. What should I automate next?
