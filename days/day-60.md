# Day 60: Analyze ZAP Findings

## Goal
Learn the core ideas for **analyze zap findings** and apply them to the course project.

## Topics
- False positives
- Risk triage
- Evidence

## Hands-on Task
Review and classify findings

## Deliverable
A triaged DAST findings list.

## Suggested Time
- Theory: 30–45 minutes
- Hands-on: 60–120 minutes
- Review: 15 minutes

## Git Commit Suggestion
```bash
git add .
git commit -m "day 60: analyze zap findings"
```

## Notes
Use this page to record:
- what you built
- what broke
- what you fixed
- screenshots
- links to commits and pull requests

## End-of-Day Reflection
1. What did I secure today?
2. What is still risky?
3. What should I automate next?
