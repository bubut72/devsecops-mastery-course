...
2. Build the required feature or security control.
3. Capture evidence such as:
   - terminal output
   - screenshots
   - scan reports
   - links to commits
...
- `day-xx-work` for daily work
- optional pull requests into `main`

## Recommended repo enhancements
You can add these later:
- `/projects` for sample code
- `/docs` for architecture diagrams
- `/assets` for screenshots
- GitHub Actions workflows under `.github/workflows`
- badges for build, scan, and coverage status

## Publishing this repository to GitHub

### Option 1: Create an empty repo on GitHub, then push from your computer
Create a new repository on GitHub from the web UI, then run these commands locally:

```bash
git init
git add .
git commit -m "initial commit: devsecops mastery course"
git branch -M main
git remote add origin https://github.com/YOUR-USERNAME/devsecops-mastery-course.git
...
git add .
git commit -m "initial commit: devsecops mastery course"
gh repo create devsecops-mastery-course --public --source=. --remote=origin --push
```

GitHub’s current documentation confirms you can create a new repository from the web UI or with GitHub CLI, and then push an existing local project with Git commands or `gh repo create`. citeturn251092search1turn251092search0turn251092search13

## Good first GitHub improvements
- Add repository topics such as `devsecops`, `security`, `github-actions`, `kubernetes`, `spring-boot`
- Enable Issues and Projects
- Add a license
- Add branch protection for `main`
- Add a pull request template
- Add a security policy later if this becomes public
...
## License
Choose a license before making the repo public. MIT is simple for educational content. Apache-2.0 is also common when you want an explicit patent grant.
